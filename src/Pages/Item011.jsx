...
const Item011 = () => {
    ...
    const hp011 = JSON.parse(localStorage.getItem('categoryName11'));

    const [categoryName, setHomeProductName011] = useState(hp011 ? hp011 : '')
    const HandleCategoryName = (e) => {
        const homeProductName = e.target.value;
        setHomeProductName011(homeProductName);
    }
    const categoryNumber = 'j';
    const titleInfo01 = {
        categoryName,
        categoryNumber
    ...
    useEffect(() => {
        localStorage.setItem('categoryName11', JSON.stringify(categoryName));
    }, [categoryName]);

    const { data: dishes11 = [], refetch, isLoading } = useQuery({
        queryKey: ['dishes11'],
        queryFn: () => fetch(`https://recyclelib-server.vercel.app/j`)
            .then(res => res.json())
    })
        ...
        <div className='flex flex-col items-center gap-10 justify-center min-w-[700px] py-4'>
            ...
            <div className='w-full lg:w-4/5 text-sm md:text-base '>
                <div className={`bg-purple-700 flex justify-center`}>
                    <input type='text' placeholder='type here...' onChange={HandleCategoryName} defaultValue={categoryName} className='py-4 bg-purple-700 text-center font-semibold text-white text-xl' />
                </div>
                ...
                <table className="table border text-center w-full backdrop-blur-sm bg-white/30">
                    <thead >
                        <tr className='bg-purple-200 p-1'>
                            <th></th>
                            <th>Product Name</th>
                            <th>Sales Price</th>
                            <th>GP%</th>
