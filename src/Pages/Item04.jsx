...
const Item04 = () => {
    ...
    const hp04 = JSON.parse(localStorage.getItem('categoryName4'));

    const [categoryName, setHomeProductName04] = useState(hp04 ? hp04 : '')
    const HandleCategoryName = (e) => {
        const homeProductName = e.target.value;
        setHomeProductName04(homeProductName);
    }
    const categoryNumber = 'd';
    const titleInfo01 = {
        categoryName,
        categoryNumber
    ...
    useEffect(() => {
        localStorage.setItem('categoryName4', JSON.stringify(categoryName));
    }, [categoryName]);

    const { data: dishes4 = [], refetch, isLoading } = useQuery({
        queryKey: ['dishes4'],
        queryFn: () => fetch(`https://recyclelib-server.vercel.app/d`)
            .then(res => res.json())
    })
        ...
        <div className='flex flex-col items-center gap-10 justify-center min-w-[700px] py-4'>
            ...
            <div className='w-full lg:w-4/5 text-sm md:text-base '>
                <div className={`bg-gray-700 flex justify-center`}>
                    <input type='text' placeholder='type here...' onChange={HandleCategoryName} defaultValue={categoryName} className='py-4 bg-gray-700 text-center font-semibold text-white text-xl' />
                </div>
