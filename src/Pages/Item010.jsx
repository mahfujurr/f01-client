...
const Item010 = () => {
    ...
    const hp010 = JSON.parse(localStorage.getItem('categoryName10'));

    const [categoryName, setHomeProductName010] = useState(hp010 ? hp010 : '')
    const HandleCategoryName = (e) => {
        const homeProductName = e.target.value;
        setHomeProductName010(homeProductName);
    }
    ...
    useEffect(() => {
        localStorage.setItem('categoryName10', JSON.stringify(categoryName));
    }, [categoryName]);

    const { data: dishes10 = [], refetch, isLoading } = useQuery({
        queryKey: ['dishes10'],
        queryFn: () => fetch(`https://recyclelib-server.vercel.app/i`)
            .then(res => res.json())
    })
    ...
    return (
        <div className='flex flex-col items-center gap-10 justify-center min-w-[700px] py-4'>
            ...
            <div className='w-full lg:w-4/5 text-sm md:text-base '>
                <div className={`bg-rose-700 flex justify-center`}>
                    <input type='text' placeholder='type here...' onChange={HandleCategoryName} defaultValue={categoryName} className='py-4 bg-rose-700 text-center font-semibold text-white text-xl' />
                </div>
