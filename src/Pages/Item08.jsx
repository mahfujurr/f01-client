...
const Item08 = () => {
    ...
    const hp08 = JSON.parse(localStorage.getItem('categoryName8'));

    const [categoryName, setHomeProductName08] = useState(hp08 ? hp08 : '')
    const HandleCategoryName = (e) => {
        const homeProductName = e.target.value;
        setHomeProductName08(homeProductName);
    }
    ...
    useEffect(() => {
        localStorage.setItem('categoryName8', JSON.stringify(categoryName));
    }, [categoryName]);

    const { data: dishes8 = [], refetch, isLoading } = useQuery({
        queryKey: ['dishes8'],
        ...
        <div className='flex flex-col items-center gap-10 justify-center min-w-[700px] py-4'>
            ...
            <div className='w-full lg:w-4/5 text-sm md:text-base '>
                <div className={`bg-indigo-700 flex justify-center`}>
                    <input type='text' placeholder='type here...' onChange={HandleCategoryName} defaultValue={categoryName} className='py-4 bg-indigo-700 text-center font-semibold text-white text-xl' />
                </div>
                ...
                <table className="table border text-center w-full backdrop-blur-sm bg-white/30">
                    <thead >
                        <tr className='bg-indigo-200 p-1'>
                            <th></th>
                            <th>Product Name</th>
                            <th>Sales Price</th>
                            <th>GP%</th>
