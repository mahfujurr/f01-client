...
const Item03 = () => {
    ...
    const hp03 = JSON.parse(localStorage.getItem('categoryName3'));

    const [categoryName, setHomeProductName03] = useState(hp03 ? hp03 : '')
    const HandleCategoryName = (e) => {
        const homeProductName = e.target.value;
        setHomeProductName03(homeProductName);
    }
    const categoryNumber = 'c';
    const titleInfo01 = {
        categoryName,
        categoryNumber
    ...
    useEffect(() => {
        localStorage.setItem('categoryName3', JSON.stringify(categoryName));
    }, [categoryName]);

    const { data: dishes3 = [], refetch, isLoading } = useQuery({
        queryKey: ['dishes3'],
        queryFn: () => fetch(`https://recyclelib-server.vercel.app/c`)
            .then(res => res.json())
    })
        ...
        <div className='flex flex-col items-center gap-10 justify-center min-w-[700px] py-4'>
            ...
            <div className='w-full lg:w-4/5 text-sm md:text-base '>
                <div className={`bg-sky-700 flex justify-center`}>
                    <input type='text' placeholder='type here...' onChange={HandleCategoryName} defaultValue={categoryName} className='py-4 bg-sky-700 text-center font-semibold text-white text-xl' />
                </div>
