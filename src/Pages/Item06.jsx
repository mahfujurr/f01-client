...
const Item06 = () => {
    ...
    const hp06 = JSON.parse(localStorage.getItem('categoryName6'));

    const [categoryName, setHomeProductName06] = useState(hp06 ? hp06 : '')
    const HandleCategoryName = (e) => {
        const homeProductName = e.target.value;
        setHomeProductName06(homeProductName);
    }
    const categoryNumber = 'f';
    const titleInfo01 = {
        categoryName,
        categoryNumber
    ...
    useEffect(() => {
        localStorage.setItem('categoryName6', JSON.stringify(categoryName));
    }, [categoryName]);

    const { data: dishes6 = [], refetch, isLoading } = useQuery({
        queryKey: ['dishes6'],
        queryFn: () => fetch(`https://recyclelib-server.vercel.app/f`)
            .then(res => res.json())
    })
        ...
        <div className='flex flex-col items-center gap-10 justify-center min-w-[700px] py-4'>
            ...
            <div className='w-full lg:w-4/5 text-sm md:text-base '>
                <div className={`bg-lime-700 flex justify-center`}>
                    <input type='text' placeholder='type here...' onChange={HandleCategoryName} defaultValue={categoryName} className='py-4 bg-lime-700 text-center font-semibold text-white text-xl' />
                </div>
